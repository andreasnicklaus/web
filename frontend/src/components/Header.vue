  ...
  <header>
    <div id="header-content" class="row between">
      ...
      <div class="row right-sided col-3 nowrap center">
        <ThemeLightDarkIcon class="material-icons clickable" :size="36" title="Change the theme"
                            v-on:click="change_theme()"/>
        <router-link v-if="username == null" class="material-icons clickable" to="/login">
          <LoginIcon title="Login" :size="36"/>
        </router-link>
        <i v-if="username != null" class="material-icons md-36 clickable" title="Log out" v-on:click="logout">logout</i>
        <span v-if="username != null && username !== undefined">{{ username }}</span>
...
export default {
  name: 'MyHeader',
  props: ['theme'],
  components: {ThemeLightDarkIcon, LoginIcon, MenuIcon, CloseIcon},
  data() {
    return {
